...
class RecetasManager {
    ...
    public function create($data, $imageFile = null) {
        try {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO Recetas (nombre, imagen, precio_venta, instrucciones, tiempo_preparacion, porciones, creado_en) 
                VALUES (?, ?, ?, ?, ?, ?, NOW())
            ");
    ...
    public function update($id, $data, $imageFile = null) {
        try {
            ...
            $stmt = $this->db->prepare("
                UPDATE Recetas 
                SET nombre = ?, imagen = ?, precio_venta = ?, instrucciones = ?, tiempo_preparacion = ?, porciones = ?
                WHERE id = ?
            ");
    ...
    // Nuevo método para reporte: Recetas por Menú
    public function getRecetasPorMenu() {
        try {
            $db = Database::getInstance()->getConnection();
            
            $stmt = $db->prepare("
                SELECT m.nombre AS menu, r.nombre AS receta, r.precio_venta, r.costo_total, 
                       (r.precio_venta - r.costo_total) AS ganancia, r.porciones
                FROM Recetas r
                JOIN MenuRecetas mr ON r.id = mr.receta_id
    ...
    public function getRentabilidadRecetas() {
        try {
            ...
            $stmt = $db->prepare("
                SELECT nombre, precio_venta, costo_total, 
...
<body>
                ...
                <div class="p-3">
                    ...
                    <nav class="nav flex-column">
                        ...
                        <a class="nav-link" href="#" data-section="recetas">
                            <i class="fas fa-book me-2"></i>Recetas
                        </a>
                ...
                <form id="ingredientesForm" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="descripcion" id="ingredienteDescripcion" rows="3"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                ...
                <form id="recetasForm" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="instrucciones" id="recetaInstrucciones" rows="5"></textarea>
                        </div>
                            ...
                            <div class="card bg-light p-3 mb-3">
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-control form-control-sm" id="selectIngrediente" onchange="showIngredientImage(this.value)">
    ...
    <script>
        ...
        function setupSearchFunctionality() {
            // Configurar búsqueda para cada tabla (actualizado para nuevas columnas con imagen)
            const searchConfigs = [
                { inputId: 'searchMenu', tableId: 'menuTable', columns: [1] }, // columna nombre (después de imagen)
                { inputId: 'searchCategorias', tableId: 'categoriasTable', columns: [1, 2] }, // nombre y descripción (después de imagen)
                { inputId: 'searchIngredientes', tableId: 'ingredientesTable', columns: [1, 2] }, // nombre y categoría (después de imagen)
                { inputId: 'searchRecetas', tableId: 'recetasTable', columns: [1] } // nombre (después de imagen)
            ];
            ...
            fetch('?action=get_data&table=recetas')
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        ...
                        const margenClass = ganancia > 0 ? 'text-success' : 'text-danger';
                        ...
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-info" onclick="viewReceta(${item.id})" title="Ver detalles">
        ...
        // Cargar ingredientes para selects
            ...
            fetch('?action=get_data&table=ingredientes')
                ...
                .then(data => {
                    ...
                    data.forEach(ing => {
                        ...
                        select.innerHTML += `<option value="${ing.id}" data-imagen="${ing.imagen || ''}">${ing.nombre} (${ing.presentacion})${imageBadge}</option>`;
                    });
                });
        }
        ...
        function showIngredientImage(ingredienteId) {
            ...
            if (ingredienteId && ingredientes.length > 0) {
                ...
                if (ingrediente && ingrediente.imagen) {
                ...
                .then(data => {
                    ...
                    switch(table) {
                        ...
                        case 'recetas':
                            ...
                            // Debug
                            console.log('Editando receta ID:', item.id);
                            console.log('currentRecetaId establecido a:', currentRecetaId);
            ...
            fetch(`?action=get_data&table=receta_detalle&id=${recetaId}`)
                ...
                .then(data => {
                    ...
                    if (data && data.ingredientes && data.ingredientes.length > 0) {
                        // Mostrar encabezados - MODIFICADO: Agregamos columna "Precio Compra"
                        container.innerHTML = `
                            <div class="row mb-2 fw-bold">
                                <div class="col-md-3">Ingrediente</div>
                                <div class="col-md-2">Cantidad</div>
                                <div class="col-md-2">Precio Compra</div> <!-- Nueva columna -->
                        ...
                        data.ingredientes.forEach(ing => {
                            ...
                            // MODIFICADO: Agregamos celda para el precio de compra
                            container.innerHTML += `
                                <div class="row mb-2 align-items-center" data-ingrediente-id="${ing.id}">
                                    ...
                                    <div class="col-md-2">
                                        <button type="button" class="btn btn-danger btn-sm w-100" onclick="removeIngredienteReceta(${ing.id})">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </div>
                                </div>
                        ...
                        container.innerHTML += `
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <strong class="text-primary fs-5">${costoTotal.toFixed(2)}</strong>
                                </div>
                            </div>
                        `;
        ...
        function finalizarReceta() {
            ...
            loadRecetasData();
            loadDashboardData();
            alert('Receta finalizada con éxito');
        }
        ...
        function debugInfo() {
            const debugData = {
                ...
                selectIngrediente: document.getElementById('selectIngrediente').value,
                cantidadIngrediente: document.getElementById('cantidadIngrediente').value,
                porcentajeMerma: document.getElementById('porcentajeMerma').value,
                ingredientesCount: ingredientes.length,
                categoriasCount: categorias.length
            };
            
            console.log('Debug Info:', debugData);
            alert('Debug info mostrado en consola (F12 -> Console)');
        }
        ...
        function agregarIngredienteReceta() {
            ...
            .then(text => {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        alert(data.message || 'Ingrediente agregado con éxito');
                    } else {
                        alert('Error al agregar ingrediente: ' + (data.error || 'Error desconocido'));
                    }
                } catch (e) {
            ...
            .catch(error => {
                ...
                alert('Error de conexión al agregar ingrediente');
            })
            .finally(() => {
                // Restaurar botón
        ...
        function removeIngredienteReceta(recetaIngredienteId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Error al eliminar ingrediente: ' + (data.error || 'Error desconocido'));
                }
            })
            .catch(error => {
                console.error('Error:', error);
        ...
        function updateIngredienteReceta(recetaIngredienteId, cantidad, porcentajeMerma) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Error al actualizar ingrediente: ' + (data.error || 'Error desconocido'));
                    loadRecetaIngredientes(currentRecetaId); // Recargar para restaurar valor anterior
                }
            })
            .catch(error => {
        ...
        function viewReceta(id) {
            // Esta función puede expandirse para mostrar una vista detallada de la receta
            editItem('recetas', id);
        }
        ...
        function loadRecetasPorMenuReport() {
            ...
            container.innerHTML = `
                <div class="text-center py-4">
                    ...
                    <p class="mt-2">Cargando reporte de recetas por menú...</p>
                </div>
            `;
        ...
        function loadIngredientesPorCategoriaReport() {
            ...
            container.innerHTML = `
                <div class="text-center py-4">
                    ...
                    <p class="mt-2">Cargando reporte de ingredientes por categoría...</p>
                </div>
            `;
        ...
        function loadRentabilidadRecetasReport() {
            ...
            container.innerHTML = `
                <div class="text-center py-4">
                    ...
                    <p class="mt-2">Cargando reporte de rentabilidad de recetas...</p>
                </div>
            `;
